...
async function getBeaconMetaForSOSIds(sosIds: string[]): Promise<Map<string, { beacon_id?: string; beacon_name?: string; source?: string }>> {
  const ids = Array.from(new Set(sosIds.filter(Boolean)));
  const bySosId = new Map<string, { beacon_id?: string; beacon_name?: string; source?: string }>();

  if (ids.length === 0) return bySosId;
  ...
  (data || []).forEach((row: any) => {
    if (!row?.sos_id || bySosId.has(row.sos_id)) return;
    const meta = row.meta || {};
...
function applyBeaconDisplay<T extends { id: string; email?: string; name?: string; location?: any }>(event: T, beaconMeta?: { beacon_id?: string; beacon_name?: string; source?: string }) {
  ...
  if (!resolvedBeaconMeta) return event;
